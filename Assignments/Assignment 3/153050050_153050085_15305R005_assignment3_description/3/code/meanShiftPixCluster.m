...
xPad = padarray(x,[height,width,0]);
disp(size(xPad));
...
%                 if iter==0
%                     fprintf('%d %d %d %d\n',height+i,2*height+i-1,width+j,2*width+j-1);
%                 end
                xDiffSq = (y-xThis).^2;
                % feature kernel weight
                intensityKernel = repmat( prod( reshape( weight_map( xDiffSq+1 ), height, width, depth) , 3 ), [1,1, depth]);
                %intensityKernel = repmat( reshape( weight_map( xDiffSq+1 ), height, width, depth) , [1,1, depth]);
                % mixed kernel weight
    ...
    if plotOn
        ...
        subplot(122), plot(0:iter, MS ), xlabel(sprintf('iteration %d',iter)), ylabel('averaged mean shift');axis square
        drawnow
    end
    if yMS <= th || iter==1 % exit if converge
        done = 1;
